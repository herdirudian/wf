    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-foreground">Arrival List</h1>
          <p className="text-sm text-muted">Daftar tamu yang dijadwalkan check-in.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground">
            ...
            <tbody className="divide-y divide-border">
              {arrivals.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-muted italic">Tidak ada kedatangan untuk tanggal ini.</td>
                </tr>
              ) : (
                arrivals.map((b) => (
                  <tr key={b.id} className="hover:bg-muted/5">
                    ...
                    <td className="px-6 py-4">
                      <span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-widest ${b.status === 'checked_in' ? 'bg-blue-100 text-blue-600' : 'bg-emerald-100 text-emerald-600'}`}>
                        {b.status === 'checked_in' ? 'In-House' : 'Pending CI'}
                      </span>
                    </td>
                  </tr>
                ))
