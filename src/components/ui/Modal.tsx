...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#2D3E10]/40 p-2 sm:p-4 backdrop-blur-sm animate-in fade-in duration-500">
      <div 
        ...
        <div className="flex min-h-[300px] sm:min-h-[400px] max-h-[calc(100dvh-2rem)] sm:max-h-[calc(100dvh-4rem)] flex-col overflow-hidden rounded-3xl sm:rounded-[2.5rem] border border-[#E8E8E1] bg-white shadow-2xl shadow-[#2D3E10]/20">
          ...
          <div className="relative flex items-center justify-between border-b border-[#E8E8E1]/60 px-4 py-3.5 sm:px-8 sm:py-5 bg-[#F1F3EE]/30">
            <div className="relative z-10">
              <h3 className="text-xs sm:text-sm font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] text-[#2D3E10]">{title}</h3>
            </div>
            <button
              type="button"
              onClick={onClose}
              className="group relative flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-white text-[#2D3E10]/60 shadow-sm transition-all hover:bg-primary hover:text-white hover:rotate-90 border border-[#E8E8E1]"
            >
